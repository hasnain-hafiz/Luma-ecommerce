...
public class PaymentWebhookService {
  ...
  private void markPaid(OrderEntity order) { order.markPaid(); orders.save(order); reservations.findByOrderId(order.getId()).forEach(reservation -> { reservation.commit(); reservations.save(reservation); }); }
  ...
  private void convertDraft(java.util.UUID draftId) {
    ...
    draft.convert(); drafts.save(draft); order.markPaid(); orders.save(order);
  }
}
