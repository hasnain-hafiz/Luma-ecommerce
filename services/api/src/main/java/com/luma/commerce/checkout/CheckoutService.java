...
public class CheckoutService {
  ...
  public CheckoutContracts.CheckoutResponse start(UUID userId, CheckoutContracts.StartCheckoutRequest request) {
    ...
    var total = new CheckoutContracts.AuthoritativeTotal(subtotal, 0, 0, subtotal, "INR"); var expiresAt = Instant.now().plusSeconds(900); var draft = drafts.save(CheckoutDraftEntity.open(userId, request.shippingAddress(), total, expiresAt));
