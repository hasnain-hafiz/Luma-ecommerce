...
class CheckoutDraftCancellationTest {
  @Test
  void cancellationReleasesReservationAndMarksDraftCancelled() {
    var reservations = mock(InventoryReservationRepository.class); var products = mock(ProductRepository.class); var orders = mock(OrderRepository.class); var drafts = mock(CheckoutDraftRepository.class); var draft = CheckoutDraftEntity.open(UUID.randomUUID(), new CheckoutContracts.ShippingAddress("Name", "Line", null, "City", "Region", "00000", "US"), new CheckoutContracts.AuthoritativeTotal(100, 0, 0, 100, "USD"), Instant.now().plusSeconds(60)); var reservation = InventoryReservationEntity.reserveForDraft(draft.getId(), UUID.randomUUID(), 1, Instant.now().plusSeconds(60));
    when(reservations.findByDraftId(draft.getId())).thenReturn(List.of(reservation)); when(drafts.findById(draft.getId())).thenReturn(Optional.of(draft));
    new InventoryReservationService(reservations, products, orders, drafts).cancelDraft(draft.getId());
    assertEquals("RELEASED", reservation.getStatus()); assertEquals(CheckoutDraftEntity.DraftStatus.CANCELLED, draft.getStatus());
  }
}
