...
export function authenticatedHeaders(contentType = false): HeadersInit {
...
export async function orderApi<T>(path: string): Promise<T> {
  ...
  const response = await fetch(`${base}/api/v1${path}`, { credentials: "include", headers: authenticatedHeaders() });
